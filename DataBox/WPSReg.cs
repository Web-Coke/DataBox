...
namespace DataBox
{
    public static class WPSReg
    {
        public static bool ISWPS =>
            (
                FileVersionInfo
                    .GetVersionInfo(Process.GetCurrentProcess().MainModule.FileName ?? string.Empty)
                    .OriginalFilename ?? string.Empty
            ).Equals("ET.EXE", StringComparison.OrdinalIgnoreCase);

        private static readonly string XllPath = ExcelDnaUtil.XllPath;
        ...
        public static void WPSRegistry(int Num = 2)
        {
            if (ISWPS)
            {
                RegistryKey Key = Registry.CurrentUser.OpenSubKey(
                    @"SOFTWARE\Kingsoft\Office\ET\AddinsWL\",
                    true
                );
                for (int I = 0; I < Num; I++)
                {
                    ...
                    if (Key?.GetValue(_Progid) == null || Key.GetValue(Progid) == null)
                    {
                        ...
                        if (I == Num - 1)
                        {
                            IntelliSenseAddIn.MainApplication.Application.Quit();
                        }
                    }
                }
                Key?.Close();
        ...
        {
            static readonly Guid _excelDnaNamespaceGuid = new Guid(
                "{306D016E-CCE8-4861-9DA1-51A27CBE341A}"
            );
            ...
            public static Guid Create(Guid namespaceId, string name, int version)
            {
                ...
                using (
                    HashAlgorithm algorithm =
                        version == 3 ? (HashAlgorithm)MD5.Create() : (HashAlgorithm)SHA1.Create()
                )
                {
            ...
            /// </summary>
            public static readonly Guid DnsNamespace = new Guid(
            ...
            /// </summary>
            public static readonly Guid UrlNamespace = new Guid(
            ...
            /// </summary>
            public static readonly Guid IsoOidNamespace = new Guid(
